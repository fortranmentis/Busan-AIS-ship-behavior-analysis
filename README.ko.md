...
프레임 렌더링은 완전 병렬(embarrassingly parallel) 작업입니다 — 모든 프레임은 전처리된 페이로드에만 의존하고 다른 프레임에는 전혀 의존하지 않으므로, 프로세스 풀에 그대로 매핑됩니다.
...
이 코드의 각 요소에는 이유가 있습니다.

- **워커 수** — `min(물리 코어 + 2, 논리 코어)`. 렌더링은 CPU 바운드이므로 물리 코어 수에 맞추되, +2의 약간의 초과 할당으로 워커가 메모리 할당 등으로 잠시 멈춘 사이에도 코어를 놀리지 않습니다.
- **`initializer` / `initargs`** — 페이로드(선박별 그룹, 캐싱된 배스맵, 색상 맵, bbox)는 **풀 시작 시 워커마다 1회만** pickle되어 전달되고, 이후 모듈 전역 변수에 보관됩니다. 이 방식이 없으면 태스크마다 데이터가 재전송되거나 — Windows `spawn`은 부모의 전역을 물려받지 않으므로 더 나쁘게는 — 워커마다 디스크에서 데이터를 다시 로드하게 됩니다. 폰트 상태는 프로세스별이므로 한글 폰트 등록도 여기서 다시 수행합니다.
- **`map`이 아닌 `imap`** — `map`은 *모든* 결과가 끝날 때까지 블로킹하고 결과 전체를 리스트로 만듭니다. 프레임 하나가 3000×2000×3바이트 ≈ 18 MB이므로 1000프레임이면 약 18 GB의 픽셀을 RAM에 들고 있게 됩니다. `imap`은 완성되는 프레임을 즉시 내어주는 지연(lazy) 이터레이터를 반환하므로, 메인 프로세스가 PNG로 저장하고 배열을 버리면 메모리는 워커 수 규모(O(workers))로 일정하게 유지됩니다. tqdm 진행 바도 실시간으로 갱신됩니다.
- **`imap_unordered`가 아닌 `imap`** — `imap`은 제출 순서를 보존하므로 `enumerate` 인덱스가 항상 프레임의 시간 순서와 일치하고, `frame_%05d.png` 번호가 구조적으로 올바릅니다. `imap_unordered`는 지연 시간이 약간 유리하지만 프레임 인덱스를 결과마다 따로 전달하지 않으면 영상 순서가 뒤섞입니다.
- **`chunksize=10`** — 태스크 인자가 `int` 하나뿐이라 기본 청크 크기(1)로는 태스크당 IPC 오버헤드가 지배적입니다. 10프레임씩 묶어 보내 pickle/큐 왕복 비용을 상각하되, `총 프레임 수 / 워커 수`보다 충분히 작게 유지해 마지막 구간에서도 부하가 고르게 분배되도록 합니다.
- **헤드리스 렌더링** — 공통 라이브러리가 `pyplot`을 건드리기 전에 matplotlib의 `Agg` 백엔드를 강제하므로 워커가 GUI 캔버스를 열려고 시도하지 않습니다. 프레임마다 figure를 하나 만들고 닫아, 태스크 간 공유되는 그리기 상태가 없습니다.
- **구조적 내결함성** — 프레임은 임시 폴더에 바로 기록되고 FFmpeg는 풀이 모두 끝난 뒤에만 실행되므로, 실행이 중단돼도 반쯤 인코딩된 결과물이 남지 않습니다. 재실행은 멱등(idempotent)입니다.
